...
class CrossConvolutions(nn.Module):
    ...
    def __init__(self, in_channels, channels, channels_hidden=512,
        ...
        self.upsample10 = nn.Upsample(size=[c.map_size[0],c.map_size[1]])
        self.upsample21 = nn.Upsample(size=[c.map_size[0]//2,c.map_size[1]//2])
    ...
    def forward(self, x0, x1, x2):
        ...
        if(self.flag_odd):
            y0_down = nn.functional.interpolate(y0_down, size=[c.map_size[0]//2, c.map_size[1]//2])
            y1_down = nn.functional.interpolate(y1_down, size=[c.map_size[0]//4, c.map_size[1]//4])
...
class NoParallelPermute(ParallelPermute):
    #order after permutation of the channels
    def get_random_perm(self, i):
        perm = np.random.permutation(self.in_channels[i])
        perm = np.sort(perm)
        perm_inv = np.zeros_like(perm)
...
class triple_cross_coupling_layer(nn.Module):
    def __init__(self, dims_in, F_class=CrossConvolutions, F_args={},
                 clamp=5.):
        super(triple_cross_coupling_layer, self).__init__()
        channels = dims_in[0][0]
        self.ndims = len(dims_in[0])
        if channels % 3 != 0:
            raise ValueError("Input dimension of triple cross coupling layer is not divisible by 3")
        self.split_dim = [list(dims_in[i]) for i in range(len(dims_in))]
        self.split_channels = channels // 3
        
        #print("SPLIT DIM inside ", self.split_dim[0][0])
        #print("CHANNELS ", self.split_channels)

        # flow layers must handle 2 of the 3 splits of the channels
        for i in range(len(dims_in)):
            self.split_dim[i][0] = self.split_channels * 2
        #print("SPLIT DIM ", self.split_dim)
        self.glow_coupling_0 = parallel_glow_coupling_layer(dims_in=self.split_dim, F_class=CrossConvolutions, F_args=F_args, clamp=clamp)
        self.glow_coupling_1 = parallel_glow_coupling_layer(dims_in=self.split_dim, F_class=CrossConvolutions, F_args=F_args, clamp=clamp)
        self.glow_coupling_2 = parallel_glow_coupling_layer(dims_in=self.split_dim, F_class=CrossConvolutions, F_args=F_args, clamp=clamp)

    def forward(self, x, rev=False):
        n_sizes = len(x)
        self.n_sizes = n_sizes
        # dividing by image size and dividing in three
        x01_, x02_, x03_ = (x[0].narrow(1, 0, self.split_channels),
                    x[0].narrow(1, self.split_channels, self.split_channels),
                    x[0].narrow(1, 2*self.split_channels, self.split_channels))
        x11_, x12_, x13_ = (x[1].narrow(1, 0, self.split_channels),
                    x[1].narrow(1, self.split_channels, self.split_channels),
                    x[1].narrow(1, 2*self.split_channels, self.split_channels))
        x21_, x22_, x23_ = (x[2].narrow(1, 0, self.split_channels),
                    x[2].narrow(1, self.split_channels, self.split_channels),
                    x[2].narrow(1, 2*self.split_channels, self.split_channels))

        # each one has all the size of one of the three splits
        x0 = [x01_,x11_,x21_]
        x1 = [x02_,x12_,x22_]
        x2 = [x03_,x13_,x23_]
        ...
        if not rev:
            # 0 split and 1 split
            s01 = [torch.cat((x0[i], x1[i]), 1) for i in range(n_sizes)]
            s01 = self.glow_coupling_0.forward(s01)
            s0, s1 = self._split_in_half(s01)
            # 1 split obtained by glow 0 and 2 plit
            s12 = [torch.cat((s1[i], x2[i]), 1) for i in range(n_sizes)]
            s12 = self.glow_coupling_1.forward(s12)
            s1, s2 = self._split_in_half(s12)
            # 0 obtained by glow 0 and 2 obtained by glow 1
            s02 = [torch.cat((s0[i], s2[i]), 1) for i in range(n_sizes)]
            s02 = self.glow_coupling_2.forward(s02)
            s0, s2 = self._split_in_half(s02)
            # I have to reconstruct y as it was x
            y = [torch.cat((s0[i], s1[i], s2[i]), 1) for i in range(n_sizes)]
        else:
            # 0 split 2 split REVERSE
            s02 = [torch.cat((x0[i], x2[i]), 1) for i in range(n_sizes)]
            s02 = self.glow_coupling_2.forward(s02, rev=True)
            s0, s2 = self._split_in_half(s02)
            # 1 split 2 split REVERSE
            s12 = [torch.cat((x1[i], s2[i]), 1) for i in range(n_sizes)]
            s12 = self.glow_coupling_1.forward(s12, rev=True)
            s1, s2 = self._split_in_half(s12)
            # 0 split and 1 split REVERSE
            s01 = [torch.cat((s0[i], s1[i]), 1) for i in range(n_sizes)]
            s01 = self.glow_coupling_0.forward(s01, rev=False)
            s0, s1 = self._split_in_half(s01)
            # I have to reconstruct y as it was x
            y = [torch.cat((s0[i], s1[i], s2[i]), 1) for i in range(n_sizes)]
    ...
    def _split_in_half(self,features):
        features_1 = [features[i].narrow(1, 0, self.split_channels) for i in range(self.n_sizes)]
        features_2 = [features[i].narrow(1, self.split_channels, self.split_channels) for i in range(self.n_sizes)]
        return features_1, features_2
            

    def jacobian(self, x, rev=False):
        jac0 = self.glow_coupling_0.last_jac[0] + self.glow_coupling_1.last_jac[0] + self.glow_coupling_2.last_jac[0]
        jac1 = self.glow_coupling_0.last_jac[1] + self.glow_coupling_1.last_jac[1] + self.glow_coupling_2.last_jac[1]
        jac2 = self.glow_coupling_0.last_jac[2] + self.glow_coupling_1.last_jac[2] + self.glow_coupling_2.last_jac[2]
